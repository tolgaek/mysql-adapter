...
exports.initialize = function initializeSchema(schema, callback) {
    ...
    if (s.pool) {
        connectionSettings.connectionLimit = s.connectionLimit || 10;
        connectionSettings.queueLimit = s.queueLimit || 0;
        connectionSettings.waitForConnections = s.wailForConnections || true;

        schema.client = mysql.createPool(connectionSettings);
        schema.client.on('connection', function (connection) {
                ...
                if (err) {
                    if (err.message.match(/(^|: )unknown database/i)) {
                        ...
                        connection.query(q, function (error) {
                            if (!error) {
                                connection.query('USE ' + s.database);
                            } else {
                                throw error;
                            }
                        });
        ...
        schema.client = mysql.createConnection(connectionSettings);
    }
